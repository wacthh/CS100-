...
struct brainfuck_state* brainfuck_state_new() {
  struct brainfuck_state* state = (struct brainfuck_state*) malloc(sizeof(struct brainfuck_state));
  state->memory_buffer = (uint8_t*) malloc(30000 * sizeof(uint8_t));
  memset(state->memory_buffer, 0, 30000 * sizeof(uint8_t));
  state->memory_pointer_offset = 0;
  state->loop_buffer = NULL;
...
void brainfuck_execute_plus(struct brainfuck_state* state) {
  state->memory_buffer[state->memory_pointer_offset] = (state->memory_buffer[state->memory_pointer_offset] + 1) % (UINT8_MAX + 1);
...
void brainfuck_execute_minus(struct brainfuck_state* state) {
  uint8_t* memory = state->memory_buffer;
  size_t offset = state->memory_pointer_offset;

  if (memory[offset] == 0) {
    memory[offset] = UINT8_MAX;
  } else {
    memory[offset]--;
  }
...
void brainfuck_execute_input(struct brainfuck_state* state, brainfuck_input_handler input_handler) {
  ...
  if (input == EOF) {
    ...
    state->memory_buffer[state->memory_pointer_offset] = (uint8_t) input;
  }
...
// ↓↓↓ FOR PART B ONLY ↓↓↓
//
bool brainfuck_execute_loop_start(struct brainfuck_state* state) {
  if (state->memory_buffer[state->memory_pointer_offset] == 0) {
    size_t unmatched_depth = 1;
    while (unmatched_depth > 0) {
      state->instruction_pointer++;
      if (state->instruction_pointer >= strlen(state->program)) {
        return false;
      }
      if (state->program[state->instruction_pointer] == '[') {
        unmatched_depth++;
      } else if (state->program[state->instruction_pointer] == ']') {
        unmatched_depth--;
      }
    }
  }
...
bool brainfuck_execute_loop_end(struct brainfuck_state* state) {
  if (state->memory_buffer[state->memory_pointer_offset] != 0) {
    size_t unmatched_depth = 1;
    while (unmatched_depth > 0) {
      state->instruction_pointer--;
      if (state->instruction_pointer == 0) {
        return false;
      }
      if (state->program[state->instruction_pointer] == ']') {
        unmatched_depth++;
  ...
  while (*src != '\0') {
    switch (*src) {
      ...
      case '.':
        ...
        break;
      case '[':
        if (brainfuck_execute_loop_start(state) == false) {
          return false;
        }
        break;
      case ']':
        if (brainfuck_execute_loop_end(state) == false) {
          return false;
        }
        break;
      default:
